...
router.post('/payment', async (req, res) => {
  ...
  try {
    ...
    // メール送信（Resend API — coedo-music.jpの認証済みドメインから送信）
      ...
      const htmlBody = `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e6dfd3; border-radius: 12px; background: #faf6ec; color: #1e3f4c;">
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="${payment.receiptUrl}" target="_blank" style="background-color: #3f9ab4; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block;">💳 領収書を表示する（Square公式）</a>
          </div>
          ` : ''}
